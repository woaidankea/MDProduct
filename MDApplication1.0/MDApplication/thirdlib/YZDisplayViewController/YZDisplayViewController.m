...
- (void)setUpTitleWidth
{
    ...
    if (totalWidth > YZScreenW-ButtonWidth) {
    ...
    CGFloat titleMargin = (YZScreenW - totalWidth-ButtonWidth
                           ) / (count + 1);
...
- (void)setUpAllTitle
{
    ...
    CGFloat buttonW = cellButtonWidth;
    CGFloat buttonH = ButtonHeight;
    CGFloat buttonX = (YZScreenW-270)/2;
    CGFloat buttonY = 40;
...
//        cellLabel.textColor = self.norColor;
        ...
        // 设置按钮标题
        label.text = vc.title;
        cellLabel.text = vc.title;
        
        labelW = [self.titleWidths[i] floatValue];
        buttonW = cellButtonWidth;
        
        // 设置按钮位置
        UILabel *lastLabel = [self.titleLabels lastObject];
        UILabel *lastcellLabel = [self.titleButtons lastObject];
        ...
        cellLabel.frame = CGRectMake(buttonX, buttonY + (buttonH+cellborder)*(i/4), buttonW, buttonH);
        // 监听标题的点击
        UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(titleClick:)];
        [label addGestureRecognizer:tap];
        
        
        UITapGestureRecognizer *celltap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(selectClick:)];

        
        [cellLabel addGestureRecognizer:celltap];
        ...
        [self.titleButtons addObject:cellLabel];
        [_selectView addSubview:cellLabel];
...
- (void)selectClick:(UITapGestureRecognizer *)tap{
    ...
    // 选中label
    [self selectLabel:[_titleLabels objectAtIndex:i]];
...
- (void)addButtonClick:(UIButton *)sender{
    NSLog(@"Button Touch");
     ...
     _selectView.hidden =  _selectView.hidden?NO:YES;
